...
const createChatStore = (
  set: (updater: (state: ChatStore) => void) => void,
  get: () => ChatStore
): ChatStore => ({
  conversations: [],
  ...
  initializeChat: async (token: string) => {
    try {
      ...
      // Load initial REST data and seed presence list to avoid missing early presence events
      await Promise.all([
  ...
  loadMessages: async (conversationId: number, page = 0) => {
    ...
    if (!conversation) {
      // Conversations may not be loaded yet; load and retry once
    ...
    // Prefer selecting the participant different from the current user; if unknown, fall back to the first non-null
    let otherParticipant = conversation.participants.find(p => currentUserId != null && p.id !== currentUserId) 
      || conversation.participants.find(p => p.id != null);
    if (!otherParticipant) {
      set((state) => { state.isLoading = false; });
      return;
    }
    ...
    if (response.success && response.data) {
      set((state) => {
        ...
        // Always merge with existing (even for page 0) to preserve any messages already present (e.g., from WS)
        const merged = [...existing, ...pageContent];
  ...
  sendMessage: async (request: SendMessageRequest) => {
    ...
    const conversation = stateSnapshot.conversations.find(c => c.participants.some(p => p.id === request.recipientId)
    );
  ...
  addMessage: (message: ChatMessage) => {
    ...
    set((state) => {
      ...
      if (idx !== -1) {
        // Merge update (e.g., edit) into existing message
      ...
      // Update conversation preview only if this message is the latest one
      ...
      if (conversation) {
        ...
        if (isFromOther) {
          if (isActive && isChatOpen) {
            ...
            // Not active: increment unread counters only for brand-new messages
            if (idx === -1) {
  ...
  // Refresh online users from backend presence endpoint (seed and reconcile)
  refreshOnlineUsers: async () => {
    try {
      const resp = await chatServiceV2.getOnlineUsers();
      if (resp.success && Array.isArray(resp.data)) {
        set((state) => {
          state.onlineUsers = new Set(resp.data as number[]);
        });
      }
    } catch (e) {
      // ignore
...
export const useChatStoreV2 = create<ChatStore>()(
  devtools(
    persist(
      immer((set, get) => createChatStore(set, get)),
      {
        name: 'chat-store-v2',
        partialize: (state) => ({
